...
public class VFXSetParameterAction : ActionBase
{
    ...
    {
        int id = Shader.PropertyToID(parameter);
    ...
    {
        if (!Override)
            return $"Reset VFX override for parameter :'{parameter}' ({dataType})";
        else
        {
            switch (dataType)
            {

                case DataType.Bool:         return $"{visualEffect.name} {parameter} {BoolValue} ";
                case DataType.Float:        return $"{visualEffect.name} {parameter} {FloatValue} ";
                case DataType.Vector2:      return $"{visualEffect.name} {parameter} {Vector2Value} ";
                case DataType.Vector3:      return $"{visualEffect.name} {parameter} {Vector3Value} ";
                case DataType.Vector4:      return $"{visualEffect.name} {parameter} {Vector4Value} ";
                case DataType.Texture2D:    return $"{visualEffect.name} {parameter} {Texture2DValue} ";
                case DataType.Texture3D:    return $"{visualEffect.name} {parameter} {Texture3DValue} ";
                case DataType.UInt:         return $"{visualEffect.name} {parameter} {UIntValue} ";
                case DataType.Int:          return $"{visualEffect.name} {parameter} {IntValue} ";
                default:
                    return $"Set VFX <UNKNOWN> parameter :'{parameter}'";
            }
        }
    }
