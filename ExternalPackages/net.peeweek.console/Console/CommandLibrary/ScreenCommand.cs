            ...
            else
            {
                switch(args[0].ToLower())
                {
                    case "resolution":
                        ...
                        if (args.Length == 1)
                        {
                            Console.Log(name, string.Format("Current resolution is {0}x{1} at {2}Hz", Screen.width, Screen.height, r.refreshRateRatio));
                        }
                        ...
                        //         r.width = width;
                        //         r.height = height;
                        //         Screen.SetResolution(r.width,r.height,FullScreenMode.ExclusiveFullScreen, new RefreshRate() { numerator = 60, denominator = 1 });
                        //         Console.Log(name, string.Format("Setting resolution to {0}x{1} at {2}Hz", r.width, r.height, new RefreshRate() { numerator = 60, denominator = 1 }));
                        //     }
                        // }
                        ...
                        //         r.width = width;
                        //         r.height = height;
                        //        // string refreshRatioString = r.refreshRateRatio.ToString();
                        //        // float refreshRateFloat = float.Parse(refreshRatioString, CultureInfo.InvariantCulture);
                        //         //var refreshRate = Math.Round(refreshRateToFloat);
                        //         r.refreshRateRatio = rate;
                        //         Screen.SetResolution(r.width, r.height, FullScreenMode.ExclusiveFullScreen, new RefreshRate() { numerator = 60, denominator = 1 });
                        //         Console.Log(name, string.Format("Setting resolution to {0}x{1} at {2}Hz", r.width, r.height, new RefreshRate() { numerator = 60, denominator = 1 }));
                        //     }
                        // }
