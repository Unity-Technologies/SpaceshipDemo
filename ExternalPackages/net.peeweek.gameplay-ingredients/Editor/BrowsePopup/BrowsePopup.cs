...
{
    public interface IBrowsePopupProvider
    {
        void CreateComponentTree(List<BrowsePopup.Element> tree);
        bool GoToChild(BrowsePopup.Element element, bool addIfComponent);
    }

    [InitializeOnLoad]
    public class BrowsePopup : EditorWindow
    {
        ...
        class Styles
        {
            ...
            public GUIStyle previewBackground = new GUIStyle("PopupCurveSwatchBackground");
        ...
        private static BrowsePopup s_FilterWindow = null;
        private static long s_LastClosedTime;
        private static bool s_DirtyList = false;

        // Member variables
        private IBrowsePopupProvider m_Provider;

        private Element[] m_Tree;
        private Element[] m_SearchResultTree;
        private List<Group> m_Stack = new List<Group>();
        ...
        private Group activeParent { get { return m_Stack[m_Stack.Count - 2 + m_AnimTarget]; } }
        ...
        void Init(Vector2 position, IBrowsePopupProvider provider)
        {
            m_Provider = provider;
        ...
        private void CreateComponentTree()
        {
            ...
            // Rebuild stack
            if (m_Stack.Count == 0)
                m_Stack.Add(m_Tree[0] as Group);
            else
            {
                // The root is always the match for level 0
                Group match = m_Tree[0] as Group;
                int level = 0;
                while (true)
                {
                    // Assign the match for the current level
                    Group oldElement = m_Stack[level];
                    m_Stack[level] = match;
                    ...
                    if (childMatch != null && childMatch is Group)
                    {
                        match = childMatch as Group;
                    }
                    else
                    {
        ...
        private void RebuildSearch()
        {
            ...
            // Add parent
            tree.Add(new Group(0, kSearchHeader));
            // Add search results
            ...
            m_Stack.Clear();
            m_Stack.Add(m_SearchResultTree[0] as Group);
        ...
        private Group GetElementRelative(int rel)
        {
            int i = m_Stack.Count + rel - 1;
            if (i < 0)
                return null;
            return m_Stack[i] as Group;
        }
        ...
        private void ListGUI(Element[] tree, float anim, Group parent, Group grandParent)
        {
        ...
        private void GoToChild(Element e, bool addIfComponent)
        {
            ...
            else if (!hasSearch)//TODO RF || e is NewElement)
            {
                ...
                {
                    m_Anim = 0;
                    m_Stack.Add(e as BrowsePopup.Group);
                }
            }

        }

        private void ListGUI(Element[] tree, Group parent)
        {
            ...
            for (int i = 0; i < children.Count; i++)
            {
                ...
                if (Event.current.type == EventType.Repaint)
                {
                    GUIStyle labelStyle = (e is Group) ? s_Styles.groupButton : s_Styles.componentButton;
